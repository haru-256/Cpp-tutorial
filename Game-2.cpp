...
void Taiketu_basyo::taiketu() //「ヒーローと大魔王が対決する」関数の定義
{
    ...
    while (you.get_power() > 0 && bu.get_power() > 0)
    {
        ...
        if (bu_no_basho == iti)
        {                                   //もしbu_no_bashoとitiが一致したら（つまり、ねらったところに大魔王がいたら）
